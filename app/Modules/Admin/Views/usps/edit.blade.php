...
@section('content')
    ...
    <div class="card card-block">
        <form method="POST" action="{{ route('admin::usps.update', ["id" => $usps->id]) }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            <usps-form
        ...
        <form class="mt-1" method="POST" action="{{ route('admin::usps.delete', $usps->id) }}">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
