<template>
  <div id="course-page" class="container p-2">
    <template v-if="school && order.courses">
      <template v-if="user.role_id == 1">
        <div class="row-flex">
          <div class="col-xs-12 col-md-7 order-2 order-md-1 checkout-left-space">
          ...
          <div class="col-xs-12 col-md-5 order-1 order-md-2 checkout-step cart-container">
            <checkout-cart
    ...
    <p class="text-xs text-muted">Köpet genomförs via Körkortsjakten AB.</p>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async checkGiftCard() {
      ...
      let response = await Api.checkGiftCard(this.pendingToken, this.courseIds.length ? this.courseIds[0] : null);
  ...
  mounted () {
    document.body.classList.add('bg-white-custom')
  },
  updated() {
  ...
  destroyed () {
    document.body.classList.remove('bg-white-custom')
  }
}
</script>
