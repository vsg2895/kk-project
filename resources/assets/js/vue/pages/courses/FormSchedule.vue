<template>
  <div>
    ...
    <div class="form-group" hidden="hidden">
      ...
      <input type="hidden" name="vehicle_segment" v-model="selectedVehicleSegment">
      <input type="hidden" name="vehicle_segment" v-model="selectedCity">
    </div>
    ...
    <div class="form-group">
      ...
      <div>
        <el-date-picker v-model="course.start_time" type="datetime" :picker-options="pickerOptions" format="yyyy-MM-dd HH:mm"
                        placeholder="Välj datum och tid" :readonly="true"></el-date-picker>
      </div>
...
<script type="text/babel">
    ...
    export default {
        ...
        watch: {
            ...
            course(value) {
              var that = this;
              if (Object.keys(that.course).length === 0) {
                return;
              }

              this.selectedCity = _.find(this.cities, (city) => {
                return city.id === that.course.city_id;
              });

              this.selectedVehicleSegment = _.find(this.vehicleSegments, (vehicleSegment) => {
                return vehicleSegment.id === that.course.vehicle_segment_id;
              });

              this.selectedSchool = _.find(this.schools, (school) => {
                return school.id === that.course.school.id;
              });
            }
        },
        components: {
        ...
        data() {
            ...
            this.selectedSchool = _.find(this.schools, (school) => {
              return school.id === this.course.school.id;
            });

            // this.initialSchool = this.course.school;
            return {
                pickerOptions: {
                ...
                cities: [],
                // selectedCity: _.find(this.cities, (city) => {
                //   return city.id === this.course.city_id;
                // }),
                schools: [],
                // selectedSchool: this.course.school,
                vehicleSegments: [],
                // selectedVehicleSegment: _.find(this.vehicleSegments, (vehicleSegment) => {
                //   return vehicleSegment.id === this.course.vehicle_segment_id;
                // }),
                startTime: null,
                initialized: false,
